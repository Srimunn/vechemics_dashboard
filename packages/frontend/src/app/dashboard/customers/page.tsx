...
import { Users, RefreshCw, IndianRupee, Search } from 'lucide-react';
...
export default function CustomersPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${backendUrl}/api/analytics/customers?from=${fromDate}&to=${toDate}`, { headers });
      if (res.ok) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-[#64748B]">Customer sales history and pending receivables balances.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <ExportButton moduleName="customers" label="Export" fromDate={fromDate} toDate={toDate} />
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold text-gray-900">All Customers ({data?.totalCustomers || 0})</h2>
          ...
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="bg-gray-50 text-xs uppercase text-gray-500 border-b border-gray-200">
              <tr>
                ...
                <th className="px-4 py-3 text-right">Invoices Billed</th>
                <th className="px-4 py-3 text-right">Total Invoiced Sales</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {filtered.map((c: any, idx: number) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right font-semibold text-gray-900">{formatINR(c.totalSales)}</td>
                  <td className="px-4 py-3 text-right font-bold text-rose-600">
                    {c.outstanding > 0 ? formatINR(c.outstanding) : '₹0'}
                  </td>
                </tr>
              ))}
            </tbody>
